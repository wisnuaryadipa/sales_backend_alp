...
class Controller {
    ...
    baseFilter = async (filterQuery: any) => {
        const lastStockOpname = await services.warehouse.stockOpname.getLatestItemOpnameBeforeDate(filterQuery.itemId, filterQuery.dateEnd, filterQuery.warehouseId);
    ...
    stockByDateRangeByWarehouse = async (dateEnd?: Date, dateStart?: Date, itemId?: string, warehouseId?: number, transactionType?: string) => {
        ...
        */
        ...
        const lastStockOpname = await services.warehouse.stockOpname.getLatestItemOpnameBeforeDate(itemId, dateEnd, warehouseId);
        const filterQuery = {
            itemId: itemId,
            dateStart: dateStart,
            dateEnd: dateEnd,
